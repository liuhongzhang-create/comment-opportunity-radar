...
3. **开始抓取** —— 工具打开每个作品页、缓慢滚动评论区，把评论抓下来，然后**自动接上分析**。
...
所以这个工具**不伪造任何请求**：它启动一个真实的 Chrome（用系统已装的 Chrome，不是自动化专用内核）加一个独立持久化配置，让**页面自己去发它本来就会发的请求**，然后从页面已经收到的响应里把评论 JSON 读出来。滚动节奏是随机化的（1.3–2.6 秒），不会高频轮询。
