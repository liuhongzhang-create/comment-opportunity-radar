...
2. **选作品** —— 两种方式任选：刷新作品列表后勾选自己的作品；或直接把**任意作品的链接**（别人的也行）粘进「或者：粘贴作品链接」输入框，支持完整链接、`v.douyin.com` 分享短链、纯数字作品 ID，甚至直接粘抖音的分享文案，工具会自动识别其中的链接。
3. **开始抓取** —— 工具打开每个作品页、滚动评论区，把评论抓下来，然后**自动接上分析**。链接这条路不需要登录——公开作品的评论区匿名也能读，登录只是降低风控风险。

链接进来的作品没有作品列表条目，所以标题是采集时从页面自己的 `aweme/detail` 请求里学的，会自动补进导出 CSV 的「作品标题」列。
...
# 抓别人的作品：--link 接受作品链接、分享短链或整段分享文案，可重复传
.venv/bin/python tools/radar-collect.py collect --link 'https://v.douyin.com/xxxx/' --out comments.csv --max 300
```
...
- **合规**：自动化操作抖音处于其服务条款的灰色地带。抓自己作品的评论风险最低；抓别人作品的公开评论同样可行，但请只用于正当用途（找合作线索、研究反馈），不要用于骚扰、刷量或转卖数据。更稳妥的替代方案是用抖音创作者中心的「评论管理」导出自己作品的评论，再走 CSV 导入。
